...
Hash-based splitting ensures that duplicate rows are sent to the same split across runs, which is critical for reproducible model training. Training large models can take minutes or hours, so caching fitted models and transformed features becomes essential for iterative development.

## Cache training artifacts

Caching avoids redundant computation when you refit or rerun pipelines. Create `train_with_cache.py` to fit a pipeline with caching enabled:
...
# Calculate accuracy — prediction column is named "predict"
accuracy = (result[target] == result["predict"]).mean()
print(f"\nTest accuracy: {accuracy:.2%}")
print(f"Predictions sample:\n{result[[target, 'predict']].head()}")
```
...
# Calculate metrics — prediction column is named "predict"
accuracy = (result[target] == result["predict"]).mean()
...
- **Fit bottleneck**: Reduce training data size with sampling, select fewer features, use simpler model, for example Logistic Regression instead of XGBoost
...
- [Optimize pipeline performance](../performance_workflows/optimize_pipeline_performance.qmd) - Improve training and evaluation performance
