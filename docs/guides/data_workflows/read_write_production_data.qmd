...
To read from GCS instead of S3, use a `gs://` path and ensure `GOOGLE_APPLICATION_CREDENTIALS` is set. Example:
...
For public S3 buckets you can use the DuckDB backend without setting AWS credentials. Example:
...
If you see connection timeouts or file-not-found errors, then see [Troubleshoot data I/O errors](../../troubleshooting/data-io-errors.qmd). Once data is loading from cloud storage, you can transform it before execution.
...
The pipeline executes when you call `execute()`: read → filter → aggregate → return results. To persist those results back to cloud storage, write them as Parquet.
...
The same deferred execution model works with databases.

## Read from PostgreSQL

Connect to PostgreSQL to query tables and execute SQL, using the same deferred execution model you've seen with cloud files.
...
For raw SQL queries, use `pg_con.sql()` so you can run arbitrary SQL and still get a deferred expression. Example:
...
2. Execute the SQL query to fetch results from PostgreSQL. The operation runs the SQL directly on the database and returns results as a pandas DataFrame. You can also create tables in PostgreSQL from Xorq expressions.
...
4. Create a PostgreSQL table from the Xorq expression. The `create_table()` method executes the expression, creates the table with the specified name, and writes the results. Set `overwrite=True` to replace existing tables, or `overwrite=False` (default); if the table already exists, then an error is raised.

Set `overwrite=True` to replace existing tables. If the table already exists and you use `overwrite=False` (default), then an error is raised.
...
Production deployments need reliability. Network failures, timeouts, and transient errors are common in cloud I/O; add retry logic so read and write operations can recover gracefully.
...
2. If none is provided, then create a connection. This allows the function to work with or without an explicit connection parameter.
...
6. If all retry attempts fail, then raise a runtime error. This provides a clear error message indicating the operation failed after exhausting all retries.
7. Use the retry function to read data with automatic error handling. If network errors occur, then the function will retry up to 3 times with exponential backoff.

Retry network timeouts and connection resets. Don't retry authentication failures or file-not-found errors. For files too large to load at once, stream in chunks instead.
...
If you run out of memory, then reduce `chunk_size` to 10,000. To see where time and resources go, add monitoring around your I/O.
...
3. If none is provided, then create a connection, allowing flexible usage with or without an explicit connection.
...
9. Use the monitoring function to read data with automatic performance tracking. All I/O operations are logged with detailed metrics for analysis. Before going live, consider performance, security, and cost.
...
Use IAM roles on EC2/ECS instead of access keys. If using access keys, then rotate credentials every 90 days and configure least-privilege permissions. Never commit credentials to code.
...
**Capacity planning**: Start with instance memory at least 2x your dataset size. Monitor actual usage and adjust based on your specific workload. When something goes wrong, use the following troubleshooting steps.
