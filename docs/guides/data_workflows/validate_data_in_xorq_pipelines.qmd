...
2. Attempt to create the UDXF with schema validation. This raises a `ValueError` immediately at expression creation because the input schema is missing the `customer_id` column. The error message shows the expected schema vs. the actual schema, making it easy to identify missing columns or type mismatches. Once schemas match, you still need to catch bad values when the pipeline runs.
...
2. Check that all required columns exist in the schema. Extract the schema from the expression and compare required columns to actual columns. If any are absent, then raise a `ValueError` with missing column names.
...
4. Execute the filtered expression and check if any rows remain. If all rows were filtered out (empty result), then raise a `ValueError` to indicate complete validation failure. This prevents downstream processing from running on empty data.
...
6. Call the validation function with required columns. The function checks schema, filters nulls, and if validation fails, then raises exceptions, ensuring only valid data proceeds to processing.

If validation fails, then you'll see specific error messages. The examples below show the missing-column error and the no-valid-rows error so you can recognize them in your own pipelines.

**Missing columns**:

The following code uses data that is missing a required column (`status`). Running it triggers a clear error listing the missing columns:
...
This example uses data where every row has nulls in the required columns. Running it shows the error you get when validation filters out all rows:
...
Beyond nulls, values can be out of range. Filter so numbers and dates stay within expected bounds.
...
Range validation removes rows that don't meet the criteria. To verify, check that the output contains only values within the specified ranges and test edge cases such as age=18 or age=120. For text fields, validate format with string methods.
...
3. Filter data to check for valid email patterns using string methods. The `.contains()` method checks if a substring exists in each string value. Multiple predicates ensure emails have both "@" and "." characters. For production, use more comprehensive validation, for example regex patterns or email validation libraries.
...
The script filters out emails that lack an "@" or a "." character. For production, use more comprehensive email validation rules. To enforce business rules that go beyond format, use a custom validation function in a UDXF.
...
3. Validate business rules using pandas operations. Filter rows where the `status` column contains valid values using `.isin()`. Rows with invalid statuses, for example "invalid", are removed from the result.
4. If all rows are filtered out, then raise an error. If no valid rows remain after validation, then raise a `ValueError` to prevent downstream processing from running on empty data.
...
7. Create a UDXF with schema validation. Schema validation happens at expression creation: if the input schema doesn't match `maybe_schema_in`, then a `ValueError` is raised immediately. The output schema is the same as input (just filtered rows), so `maybe_schema_out=actual_schema`.
8. Execute the validated expression to apply business rule validation. Runtime validation (the `validate_customer_data` function) runs during execution, filtering out rows with invalid status values. Schema validation already occurred at expression creation. Choosing when to validate and what to monitor matters for production.
...
**Validation at expression creation**: Schema checks catch critical schema mismatches that would break your pipeline when you create the UDXF expression, before any data processing starts.

**Validation at runtime**: Filters and assertions handle null checks, range validation, and business rules that filter rows. These catch data quality issues when you call `.execute()`.
