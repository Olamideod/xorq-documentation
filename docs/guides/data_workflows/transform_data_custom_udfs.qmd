...
5. Execute the expression to materialize results. The UDF processes all rows, removing whitespace and converting to lowercase, producing a DataFrame with the original and cleaned text columns. If the UDF receives invalid data, it fails at execution time unless you validate inputs first.
...
2. Validate inputs using vectorized pandas operations. The `.any()` method checks if any value in the Series violates the constraint. If validation fails, then raise a descriptive error, preventing invalid calculations from propagating.
...
The validation catches the error at execution, preventing invalid calculations from propagating through your pipeline. For metrics across groups rather than row by row, use aggregation UDFs.

## Create aggregation UDFs

Aggregation UDFs reduce multiple rows to a single value, enabling custom metrics that SQL's built-in aggregations don't support.
...
9. Group data by category and apply the aggregation UDF. The UDF processes each group independently, calculating a weighted median that reduces the influence of outliers compared to a standard median. Test UDF logic on its own before wiring it into a pipeline.
...
# Inline the UDF functions so tests run without separate module files (see text_transform.py and validated_transform.py above).
def clean_text_fn(df: pd.DataFrame) -> pd.Series:
    """Remove extra whitespace and convert to lowercase."""
    return df["text"].str.strip().str.lower()
...
The snippets below show a simple native expression (price × quantity) versus a UDF for custom logic. Use the first when the operation is built-in; use the second when you need your own function.
...
Avoid row-by-row processing in UDFs; vectorized operations run much faster. The code below contrasts a slow `.apply()`-based UDF with a vectorized version so you can use the same pattern in your own functions.
...
Copying DataFrames inside UDFs costs memory and time. The example below shows a UDF that copies unnecessarily versus one that works on columns directly, so you can keep your UDFs lean.
...
Log UDF errors with input samples to diagnose issues. The code below wraps a UDF in try/except and logs failures with a small sample of the input so you can debug in production without dumping full datasets.
...
2. Create an updated version that adds punctuation removal. The new version extends the original functionality while keeping the old version available. If the new version causes issues, then you can roll back; this enables gradual migration.

If needed, then you can migrate gradually or roll back.
...
```
1. Convert the return value to match the declared `return_type`. Use `.astype()` to ensure the pandas Series has the correct dtype, for example "float64", that matches the UDF's declared return type.
...
5. Calculate and print the performance ratio. If the UDF is >10x slower than native operations, consider using native SQL or optimizing the UDF logic, for example vectorization or reducing data copies.
