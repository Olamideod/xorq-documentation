...
See [Switch between backends](../getting_started/switch_between_backends.qmd) and [Supported backends](../api_reference/backends/supported_backends.qmd).
...
### Issue: Pipeline fitting is slow

**Cause**: The training data is large or the model is expensive to fit.

**Solution**: Use caching to avoid re-fitting on every run:
...
See [Cache expression results](../getting_started/cache_expression_results.qmd) for details.

## Next steps

- [Backend connection issues](backend_connection_issues.qmd) for connection timeouts
- [Switch between backends](../getting_started/switch_between_backends.qmd) for backend setup
